...
def env_to_config(repository_ctx, env, relative_root = "."):
    ...
    cxx_builtin_include_directories.extend([
        ...
        "/proc/self/cwd/external/{}/".format(repository_ctx.attr.name) + paths.join(native_sysroot, "usr/lib/{tp}/gcc/{tp}".format(tp = target_prefix), gcc_ver, "include"),
        "/proc/self/cwd/external/{}/".format(repository_ctx.attr.name) + paths.join(native_sysroot, "usr/lib/{tp}/gcc/{tp}".format(tp = target_prefix), gcc_ver, "include-fixed"),
        "/proc/self/cwd/external/{}/".format(repository_ctx.attr.name) + paths.join(target_sysroot, "usr/include"),
        "/proc/self/cwd/external/{}/".format(repository_ctx.attr.name) + paths.join(target_sysroot, "usr/include/c++", gcc_ver),
        "/proc/self/cwd/external/{}/".format(repository_ctx.attr.name) + paths.join(target_sysroot, "usr/include/c++", gcc_ver, target_prefix),
    ])
